...
const GenericCard = ({ id, title, blog, imagePath }) => {
  ...
  return (
    <Card className="mt-3 main-card-blog">
      <Container>
        <Card.Header className="text-center header-card-blog">
          <b>{title}</b>
        </Card.Header>
        <Row>
        ...
        <Row>
          <Col>
            <Card.Body className="d-flex flex-column justify-content-between card-body-blog">
              <div>
                <Card.Text className="card-text-blog">{shortenedBlog}</Card.Text>
              </div>
              <div className="mt-auto">
                <Button variant="dark" className="w-100 mb-5" onClick={() => handleOnClick()}>
                  Read
                </Button>
              </div>
            </Card.Body>
