...
const DeliveryInfo = () => {
  ...
  const saveInfo = async () => {
    if (address === null || address === "") {
      ...
      try {
        await axios.post("http://localhost:5000/api/delivery/set", {
          address,
    ...
    const fetchDeliveryAddress = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:5000/api/delivery/${userInfo._id}`
        );
  ...
  return (
    <Container className="p-5">
      ...
      <Row
        style={{ height: "30vh", marginTop: "15vh" }}
